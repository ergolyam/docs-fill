...
DOCX_DIR = Path(__file__).parent / "docx_templates"
...
def load_yaml_meta(template_name: str) -> dict:
    yml = DOCX_DIR / f"{Path(template_name).stem}.yaml"
    if yml.exists():
...
async def index(request: Request):
    files = [p.name for p in DOCX_DIR.glob("*.docx")]
...
async def fill(request: Request, tpl: str):
    ...
    return templates.TemplateResponse("fill.html",
                                      {"request": request,
                                       "tpl": tpl,
                                       "fields": fields})
...
async def generate(request: Request):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        template = DocxTemplate(DOCX_DIR / tpl)
        template.render(ctx)
